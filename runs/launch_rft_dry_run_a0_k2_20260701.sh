...
OUT=/data/wzl/OpenSeeker-AgentDataFactory/results/rft-dry-run-a0-k2-20260701
...
CUDA_VISIBLE_DEVICES=0 PYTHONNOUSERSITE=1 python -m openseeker_factory.cli sample-rft-candidates \
  ...
  --disable-thinking \
  --seed 20260701 \
  2>&1 | tee /data/wzl/OpenSeeker-AgentDataFactory/logs/rft-dry-run-a0-k2-20260701-sample.log
...
PYTHONNOUSERSITE=1 python -m openseeker_factory.cli rft-filter \
  ...
  --model-label qwen3-8b-a0-restore-rft-dry-run \
  --iteration 1 \
  2>&1 | tee /data/wzl/OpenSeeker-AgentDataFactory/logs/rft-dry-run-a0-k2-20260701-filter.log
