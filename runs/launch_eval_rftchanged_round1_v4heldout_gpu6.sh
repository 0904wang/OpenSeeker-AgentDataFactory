...
CUDA_VISIBLE_DEVICES=6 PYTHONNOUSERSITE=1 python -m openseeker_factory.cli evaluate-model \
  ...
  --model-label qwen3-8b-lora-sft-rftchanged-round1-v4heldout200 \
  ...
  --local-files-only \
  --disable-thinking \
  --out-dir /data/wzl/OpenSeeker-AgentDataFactory/results/eval-qwen3-8b-sft-rftchanged-round1-v4heldout200-gpu6 \
  2>&1 | tee /data/wzl/OpenSeeker-AgentDataFactory/logs/eval-qwen3-8b-sft-rftchanged-round1-v4heldout200-gpu6.log
