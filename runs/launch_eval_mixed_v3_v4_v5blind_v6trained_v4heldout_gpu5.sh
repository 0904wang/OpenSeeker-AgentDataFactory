...
CUDA_VISIBLE_DEVICES=5 PYTHONNOUSERSITE=1 python -m openseeker_factory.cli evaluate-model \
  --samples /data/wzl/OpenSeeker-AgentDataFactory/results/heldout-eval-samples-200-canonical-v4/samples.jsonl \
  --model-label qwen3-8b-lora-sft-2p4k-mixed-v3-v4-v5blind-v6trained-v4heldout200 \
  ...
  --local-files-only \
  --disable-thinking \
  --out-dir /data/wzl/OpenSeeker-AgentDataFactory/results/eval-qwen3-8b-sft-2p4k-mixed-v3-v4-v5blind-v6trained-v4heldout200-gpu5 \
  2>&1 | tee /data/wzl/OpenSeeker-AgentDataFactory/logs/eval-qwen3-8b-sft-2p4k-mixed-v3-v4-v5blind-v6trained-v4heldout200-gpu5.log
